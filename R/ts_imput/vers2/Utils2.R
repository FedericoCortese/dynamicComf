...
temp_krige=function(y,x){
  ...
  for(i in 2:ncol(y)){
    ...
    y_sp=y[,c(1,i)]
    y_sp$cov=x[,i]
    ind_fill=which(is.na(y_sp[,2]))
    #NAcount=sum(is.na(y_sp[,2]))
    
    temp_grid=1:nrow(y)
    temp_grid=expand.grid(temp_grid,1)
    temp_grid$cov=x[,i]
    colnames(temp_grid)=c("indx","knot","cov")
    temp_grid=temp_grid[ind_fill,]
    gridded(temp_grid)=~indx+knot
    
   # if(NAcount>0){
      colnames(y_sp)=c("time","z","cov")
      y_sp$indx=1:nrow(y_sp)
      y_sp$knot=1
      y_sp=drop_na(y_sp)
      coordinates(y_sp) <- ~ indx + knot
      
      onedim_krig = autoKrige(z~cov, y_sp,temp_grid)
      y[ind_fill,i]=onedim_krig$krige_output$var1.pred#[ind_fill]
    #}
  }
  return(y)
}
