lmcovlatent.cont.MISS <- function(Y,X1=NULL,X2=NULL,yv = rep(1,nrow(Y)),k,start=0,
                             tol=10^-8,maxit=1000,
  ...
  # ---- When there is just 1 latent states ----
  if(k == 1){
    cat("Use basic model without covariates\n")
    out = lmbasic.cont.MISS(Y,k,start=start,tol=tol,maxit=maxit,
                            Mu=Mu,Si=Si)
    return(out)
  }
 ...
 # browser()
  ent = -sum(V*log(pmax(V,10^-300)))
  out = list(lk=lk,Be=Be,Ga=Ga,Mu=Mu,Si=Si,np=np,k = k,aic=aic,bic=bic,lkv=lkv, ent=ent,
             n = n, TT = TT,paramLatent=param )
  # final output
  if(miss) out$Y = Y
  if(output){
    out$PI = PI
