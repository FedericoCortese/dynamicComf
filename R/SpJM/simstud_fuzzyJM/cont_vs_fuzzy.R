...
df <- data.frame(
  Time = seq_along(soft_scen$pi_2),
  True        = soft_scen$pi_2,
  ContinuousJM = 1 - res_cont$best_S[, 2],
  FuzzyJM     = res_fuzzy$best_S[, 2]
)
...
true_S=cbind(hard_scen$pi_1, 1-hard_scen$pi_1)
...
# 2) build a data frame with Time and the three series
df <- data.frame(
  Time = seq_along(hard_scen$pi_1),
  True        = hard_scen$pi_1,
  ContinuousJM = res_cont$best_S[, 1],
